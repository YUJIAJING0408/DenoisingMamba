...
def ams_ssim(X, Y, win_size=11, win_sigma=1.5, win=None, data_range=255, size_average=True, weights=None):
    r""" interface of ms-ssim
    Args:
        X (torch.Tensor): a batch of images, (N,C,H,W)
        Y (torch.Tensor): a batch of images, (N,C,H,W)
    ...
    for i in range(scales):
        ...
        if smaller_side  > win_size:
            ...
            win = _fspecial_gauss_1d(smaller_side, win_sigma)
        win = win.repeat(X.shape[1], 1, 1, 1)
        win = win.to(X.device, dtype=X.dtype)
        ssim_val = _ssim_per_channel(X, Y,
                                     win=win,
...
class MS_SSIM(torch.nn.Module):
    ...
    def forward(self, X, Y):
        return ams_ssim(X, Y, win=self.win, size_average=self.size_average, data_range=self.data_range, weights=self.weights)


# def test_ams_ssim():
#     x = torch.randn(1, 3, 1000,1000)
#     y = torch.randn(1, 3, 1000,1000)
#     out = ams_ssim(x,y,data_range=1)
#     print(out)