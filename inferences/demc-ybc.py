...
def build_data(args):
    s = 128*16
    print(s)
    test_data = YBCDataset(data_dir="/media/yujiajing0408/Study/YCB", spp="32spp",
                              ...
                              tfs=transforms.Compose([
                                  ...
                                  transforms.Resize((s,s),antialias=False),
                              ]))
    return test_data
...
if __name__ == '__main__':
    ...
    model = DEMCModel.load_from_checkpoint(
        f'/media/yujiajing0408/Data/MD_logs/DEMC-v3-ybc/[2025-02-1:18]-|epoch=250|-|lr=0.0023|-|loss_rate=0.65:0.35|_32SPP/checkpoints/DEMC-v3-epoch=218-val_loss=0.008276.ckpt',
        strict=False,
    ).to(args.device)
    model.eval()
    model.freeze()
    ...
    a_r = []
    a_t = []
    args.out_path = r'/home/yujiajing0408/PycharmProjects/MD/outputs/DEMC-test'
    ...
    with torch.no_grad():
        with open(os.path.join(args.out_path, 'datas.txt'), 'w') as file:
            ...
            # for i in tqdm([i * 5 for i in range(63)]):
                ...
                denoise_ldr = model.forward(noisy_with_aux_features[:,:3,:,:],noisy_with_aux_features[:,3:,:,:]).squeeze(0)
